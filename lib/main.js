...
let downloadListener = {
  "onDownloadChanged": function (download)
  {
    if (download.succeeded)
    {
      ...
      {
        let downloadUri = ioService.newURI(download.source.url, null, null);

        // Remove from history.
        timers.setTimeout(removeHistoryEntry.bind(downloadUri), prefService.prefs.clearDelay * 1000);
      }, reportError).then(null, reportError);
    }
    else if (download.canceled && download.stopped)
    {
      // Finalize download, to ensure no files remain
      download.finalize().then(function ()
      {
        let downloadUri = ioService.newURI(download.source.url, null, null);
...
function removeHistoryEntry()
{
  ...
  try
  {
    browserHistory.removePage(this);
  }
  catch (removeEx)
  { }
}
