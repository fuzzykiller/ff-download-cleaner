...
var downloadListener = {
  "onDownloadStateChange": function (previousState, download)
  {
    ...
    {
      // At this point, the download should be present in the history and will be removed.
      ...
      {
        // ...or maybe, it isn't present yet. Delay 5 retries for 1s each.
        currentTimeout = timers.setTimeout(deferredDelete.bind(download, 0), 1000);
      }
    }
  },
...
// Deferred deletion procedure - in case there's something strange in the neighborhood.
function deferredDelete(iterationCount)
{
  currentTimeout = null;
  
  if (browserHistory.isVisited(this.source))
  {
    try
    {
  ...
  {
    iterationCount++;
    currentTimeout = timers.setTimeout(deferredDelete.bind(this, iterationCount), 1000);
  }
}
