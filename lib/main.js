...
{
  // Handle interval enabled preference - start and stop timer as needed. A delay of zero or less is not supported.
  if (prefService.prefs.clearOnInterval && prefService.prefs.clearInterval > 0)
  {
    clearIntervalTimer = timers.setInterval(emptyDownloadsHistory, prefService.prefs.clearInterval * 60 * 1000);
  }
  else if (clearIntervalTimer != null)
  {
    timers.clearInterval(clearIntervalTimer);
    clearIntervalTimer = null;
  }
}
// Executed when a user preference changes.
function prefChanged(prefName)
{
  if (prefName === prefNames.clearOnInterval || prefName === prefNames.clearInterval)
  {
    setClearInterval();
  }
}
